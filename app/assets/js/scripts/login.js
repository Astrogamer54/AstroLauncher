...
ipcRenderer.on('MSALoginWindowReply', (event, ...args) => {
    if (args[0] === 'error') {
        ...
        switch (args[1]){
            ...
            case 'AuthNotFinished': {
                setOverlayContent('ERROR', 'You have to finish the login process to use the Astro Launcher. The window will close by itself when you have successfully logged in.', 'OK')
                setOverlayHandler(() => {
                    toggleOverlay(false)
                    toggleOverlay(false, false, 'msOverlay')
                })
    ...
    if (queryMap.has('error')) {
        ...
        if(error === 'access_denied'){
            error = 'ERRPR'
            errorDesc = 'To use the Astro Launcher, you must agree to the required permissions! Otherwise you can\'t use this launcher with Microsoft accounts.<br><br>Despite agreeing to the permissions you don\'t give us the possibility to do anything with your account, as all information is sent to Microsoft.'
        }        
